...
Napi::Value getScreenshotSync(const Napi::CallbackInfo &info)
{
  ...
  CFDataRef rawData = getScreen(x, y, width, height);
  CFIndex length = CFDataGetLength(rawData);
  const UInt8 *rawBuffer = CFDataGetBytePtr(rawData);
  // Napi::ArrayBuffer abuf = Napi::ArrayBuffer::New(env, &rawBuffer, length);
  // memcpy(abuf, rawBuffer, length);

  Napi::Value buf = Napi::Buffer<const UInt8 *>::New(env, &rawBuffer, length);

  // memcpy(buf->GetContents().Data(), rawData, length);
